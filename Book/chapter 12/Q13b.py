"""
custom training loop with different optimizers for different layers 
"""
...
lower_optimizer = tf.keras.optimizers.Adam(learning_rate = 0.01) 
upper_optimizer = tf.keras.optimizers.SGD(learning_rate = 0.02)
loss_fn = tf.keras.losses.sparse_categorical_crossentropy
mean_loss = tf.keras.metrics.Mean()
metrics = [tf.keras.metrics.SparseCategoricalAccuracy()]


lowerLayers = tf.keras.Sequential([
    tf.keras.layers.Flatten(input_shape = [28 ,28]),
    tf.keras.layers.Dense(100, activation = "relu"),
])

upperLayer = tf.keras.Sequential([
    ...
    for epoch in epochs:
        with trange(1, n_steps + 1, desc = f"{epoch}/{n_epochs}") as steps:
            for step in steps:
                ...
                for optimizer , layer in ((lower_optimizer, lowerLayers),(upper_optimizer, upperLayer)):
